...
export default function GamePage({ currentUser = 'Emmanuel' }) {
  ...
  const isHost = mockPlayers.find((p) => p.username === currentUser)?.isHost;
  ...
  const handleSelectNumber = (num: any | React.SetStateAction<null>) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white p-6 relative">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {mockPlayers.map((player) => (
          ...
          >
            ...
            {!gameStarted && (
              <span className="absolute top-2 right-2 text-xs text-gray-300 italic">
                Waiting...
              </span>
            )}
            {winner && winner === player.username && (
              <span className="absolute top-2 right-2 text-green-400 font-bold text-xs">
                Winner 🏆
              </span>
            )}
          </div>
      ...
      {gameStarted && countdown > 0 && (
        <div className="text-center mb-6">
          <h3 className="text-3xl font-bold mb-2">Game Starts In:</h3>
          <div className="text-5xl font-mono">{countdown}</div>
        </div>
      )}

      {/* Number Selection */}
      {gameStarted && countdown === 0 && !numberSelected && (
        <div className="text-center mb-8">
      ...
      {winner && numberSelected && (
        <div className="text-center mt-10 text-2xl font-bold text-green-400">
          🎉 {winner === currentUser ? "You won!" : `${winner} won the round!`}
        </div>
      )}
    </div>
  );
